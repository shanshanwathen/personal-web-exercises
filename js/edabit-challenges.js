...
function getTotalPrice(groceries) {
    ...
    for (var i = 0; i < groceries.length; i++) {
        total = total + groceries[i].quantity * groceries[i].price;
    }
...
function windowMaxes(array, windowLength) {
    var arraySlice = [], result = [];
    for (var i = 0; i <= array.length - windowLength; i++) {
        arraySlice = array.slice(i, i + windowLength);
        result.push(Math.max(...arraySlice));
    }
    return result;
}